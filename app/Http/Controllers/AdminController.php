    ...
    */
    ...
    public function dashboard(Request $request){      
        ...
        for ($x = 0; $x < $count_package; $x++) {
            ...
            $registration = Payment::where('status','paid')->where('product_id', $product_id)->where('package_id', $package[$x]->package_id)->whereBetween('created_at', [ $from , $to ])->count();
            $paidticket = Ticket::where('ticket_type', 'paid')->where('product_id', $product_id)->where('package_id', $package[$x]->package_id)->whereBetween('created_at', [ $from , $to ])->count();
            $freeticket = Ticket::where('ticket_type', 'free')->where('product_id', $product_id)->where('package_id', $package[$x]->package_id)->whereBetween('created_at', [ $from , $to ])->count();
            $totalpackage = Payment::where('status','paid')->where('product_id', $product_id)->where('package_id', $package[$x]->package_id)->count();

            $collection = Payment::where('status','paid')->where('product_id', $product_id)->where('package_id', $package[$x]->package_id)->sum('totalprice');
