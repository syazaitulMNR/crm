...
class AdminController extends Controller
{
    ...
    */
    ...
    public function dashboard(Request $request){      
        ...
        // get the total
        $register = Payment::where('status','paid')->where('product_id', $product_id)->where('created_at', [ date('Y-m-d')])->count();
        $paid = Ticket::where('ticket_type', 'paid')->where('product_id', $product_id)->where('created_at', [ date('Y-m-d')])->count();
        $free = Ticket::where('ticket_type', 'free')->where('product_id', $product_id)->where('created_at', [ date('Y-m-d')])->count();
