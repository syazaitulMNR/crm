...
class ReportsController extends Controller
{
    ...
    */
    ...
    public function trackpackage($product_id)
    {
        ...
        $packageinfo = Payment::where('status', 'paid')->where('product_id', $product_id)->where('package_id', 'PKD0046')->whereBetween('created_at',[ date('Y-m-d 00:00:00', strtotime("-1 days")) , date('Y-m-d 23:59:59', strtotime("-1 days")) ])->get();
        $count_package = Package::where('product_id', $product_id)->count();
        ...
        for ($i = 0; $i < $count_package; $i++)
        {
            ...
            $data = Payment::where('status', 'paid')->where('product_id', $product_id)->where('package_id', 'PKD0046' )->whereBetween('created_at', [ $dari , $hingga ])->orderBy('created_at', 'desc')
                    ->get()
                    ->groupBy(function($val) {
                    return Carbon::parse($val->created_at)->format('Y-m-d');
                    });
            $data1 = Payment::where('status', 'paid')->where('product_id', $product_id)->where('package_id', 'PKD0046' )->whereBetween('created_at', [ $dari , $hingga ])->orderBy('created_at', 'desc')
                    ->get()
                    ->groupBy(function($val) {
                    return Carbon::parse($val->created_at)->format('Y-m-d');
                    })->first(); 
            $data2 = Payment::where('status', 'paid')->where('product_id', $product_id)->where('package_id', 'PKD0046' )->whereBetween('created_at', [ $dari , $hingga ])->orderBy('created_at', 'desc')
                    ->get()
                    ->groupBy(function($val) {
                    return Carbon::parse($val->created_at)->format('Y-m-d');
                    })->skip(1)->first(); 
            $totalquantity = Payment::where('status', 'paid')->where('product_id', $product_id)->where('package_id', 'PKD0046' )->whereBetween('created_at', [ $dari , $hingga ])->orderBy('created_at', 'desc')
                            ->get()
                            ->groupBy(function($val) {
                                return Carbon::parse($val->created_at)->format('d M Y');
                                });
        ...
        return view('admin.reports.trackpackage', compact('totalperpackage', 'totalpackage', 'totalquantity', 'registration', 'data', 'visitorTraffic', 'results', 'order', 'count_package', 'product', 'package', 'payment', 'student', 'counter', 'totalsuccess', 'totalcancel', 'paidticket', 'freeticket' , 'link'));
    }
