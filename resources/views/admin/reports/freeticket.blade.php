...
<div class="row py-4">     
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
        <div class="card-header" style="border: 1px solid rgb(233, 233, 233); border-radius: 5px;">
          ...
          / <a href="{{ url('viewbypackage') }}/{{ $product->product_id }}/{{ $package->package_id }}">{{ $package->name }}</a> / <b>Free Ticket</b>
        </div>
  
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 class="h2">{{ $package->name }}</h1>

          <a class="btn btn-outline-warning" href="{{ url('export-free') }}/{{ $product->product_id }}/{{ $package->package_id }}"><i class="fas fa-download pt-1 pr-1"></i> Export Participant</a>
        </div>
        ...
        @if(isset($details))
          <table class="table table-hover" id="successTable">
            ...
            <tr>
                ...
                <td>
                    <a class="btn btn-dark" href="{{ url('free-ticket/view') }}/{{ $product->product_id }}/{{ $package->package_id }}/{{ $tickets->ticket_id }}"><i class="fas fa-chevron-right"></i></a>
                    ...
                    <div class="modal fade" id="exampleModal{{ $tickets->payment_id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <a class="btn btn-danger" href="{{ url('delete-free') }}/{{ $product->product_id }}/{{ $tickets->package_id }}/{{ $tickets->ticket_id }}">Delete</a>
                            </div>
                        </div>
                        </div>
                    </div>
