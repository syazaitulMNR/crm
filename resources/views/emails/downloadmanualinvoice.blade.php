...
<div class="invoice-box">
        <table cellpadding="0" cellspacing="0">
            ...
            @foreach (array_combine($arrayfeat, $arrayquan) as $features => $quantitys)
                @foreach ($listoffeatures as $list => $listval)
                    @for ($i = 0 ; $i < count($arrayfeat) ; $i++) 
                    @if ($listval->product_features_name == $features)
                        <tr class="item">
                            <td>
                                {{ $i++ }}
                            </td>
            ...
            {{-- @foreach ($invoice as $features => $featval)
                @foreach ($listoffeatures as $list => $listval)
                    @if ($featval == $listval->product_features_name)
                    <tr class="item">
                        <td>
                            1
                        </td>
                        ...
                        <td>
                            {{ $listval->product_features_name }}<br>
                        </td>
                    ...
                    <tr>
                        ...
                        <td></td>
                            ...
                            <td>
                                ...
                                Total Taxable Amount RM {{ $inv->taxable_amount }}<br>
                                SST ({{ $inv->tax }}%) RM 566.04<br>
                                <hr style="width:35%;text-align:right;margin-right:0">
                                <strong>Total RM{{ $subtotal }}</strong><br>
                                <hr style="width:35%;text-align:right;margin-right:0">
                                <strong>Balance Due RM{{ $inv->price }}</strong><br>
                            </td>
                            @else
                            <td>
                                Sub Total RM{{ $inv->price }}<br>
